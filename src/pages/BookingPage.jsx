...
export default function BookingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    else { toast.success('Booking request sent! We\'ll contact you within 24 hours.'); setForm({ first_name:'', last_name:'', whatsapp:'', email:'', service:'', preferred_date:'', budget_range:'', vision:'' }) }
    setSaving(false)
  }
      ...
      <div style={{ minHeight:'100vh', background:'#0A0806', paddingTop:120, paddingBottom:80 }}>
        <div style={{ maxWidth:760, margin:'0 auto', padding:'0 24px' }}>
          <div style={{ textAlign:'center', marginBottom:60 }}>
            <div style={{ fontFamily:'Cinzel,serif', fontSize:9, letterSpacing:'0.4em', textTransform:'uppercase', color:'#C9A84C', marginBottom:16 }}>Get Started</div>
            <h1 style={{ fontFamily:'Cormorant Garamond,serif', fontSize:'clamp(36px,5vw,60px)', fontWeight:300, color:'#F9F4EC', marginBottom:16 }}>
              Book Your <em style={{fontStyle:'italic',color:'#C9A84C'}}>Consultation</em>
            </h1>
            <p style={{ fontFamily:'Cormorant Garamond,serif', fontStyle:'italic', fontSize:16, color:'#8A7A5A' }}>Every great outfit starts with a conversation.</p>
          </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom:16 }}>
                ...
                <select style={{...inp,appearance:'none'}} value={form.service} onChange={e=>setForm({...form,service:e.target.value})}>
                  ...
                  {['Bespoke / Custom Outfit','Ready-to-Wear Purchase','Styling Consultation','Ankara & African Print','Occasion / Aso-Ebi Order','Accessories'].map(s=><option key={s}>{s}</option>)}
                </select>
              </div>
              <div className="sbr-form-row" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
              ...
              <div style={{ marginBottom:32 }}>
                <label style={label}>Tell Us About Your Vision</label>
                <textarea style={{...inp,height:120,resize:'none'}} value={form.vision} onChange={e=>setForm({...form,vision:e.target.value})} placeholder="Describe your dream outfit, occasion, or any inspiration..." onFocus={e=>(e.target.style.borderColor='#C9A84C')} onBlur={e=>(e.target.style.borderColor='rgba(201,168,76,0.2)')} />
              </div>
              <button type="submit" disabled={saving} style={{ width:'100%', background:'linear-gradient(135deg,#E8C97A,#C9A84C)', color:'#0A0806', border:'none', padding:'18px', fontFamily:'Cinzel,serif', fontSize:10, fontWeight:600, letterSpacing:'0.3em', textTransform:'uppercase', cursor:saving?'not-allowed':'pointer', opacity:saving?0.7:1 }}>
                {saving ? 'Sending...' : 'Send Booking Request ◆ We\'ll Contact You Within 24hrs'}
              </button>
            </form>
          </div>
        </div>
